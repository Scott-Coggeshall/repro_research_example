...
cleaned_data %>% ggplot(aes(x = plate_number, y = count, color = ref_flag)) + geom_jitter(size = 2, alpha = .5, width = .2)  + theme_light() + facet_wrap( ~ plate_group, scales = "free_x") + scale_color_d3(labels = c("Other", "Copan Floq"))
...
plot(stan_model, regex_pars = "swab_type", prob_outer = .95, transformations = "exp") + geom_vline(xintercept = 1) + scale_y_discrete(labels = levels(cleaned_data$swab_type)[-1])
